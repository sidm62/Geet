...
navigator.geolocation.getCurrentPosition(position => {
  const userLats = position.coords.latitude;
  const userLons = position.coords.longitude;

  restaurants.sort((a,b) => {

    const distA = Math.sqrt
    (a.location.coordinates[0]-userLons)**2 +
      (a.location.coordinates[1]-userLats) **2 ;

    const distB = Math.sqrt(
      b.location.coordinates[0] - userLons) ** 2 + (
        b.location.coordinates[1] - userLats) ** 2
      ;
    return distA - distB;
  });
target.innerHTML = ``;
for (const restaurant of restaurants) {
  ...
  target.insertAdjacentHTML('beforeend', rivi);
}
